...
try {
    ...
    function Replace-AllStories([string]$findText, [string]$replaceText) {
        if ($findText.Length -gt 200) {
            foreach ($paragraph in $doc.Paragraphs) {
                $current = ($paragraph.Range.Text -replace '[\r\a]', '').Trim()
                if ($current -ceq $findText) {
                    $hit = $paragraph.Range.Duplicate
                    [void]$hit.MoveEnd(1, -1)
                    $hit.Text = $replaceText
                    return
                }
            }
            throw "Long source paragraph was not found: $($findText.Substring(0, [Math]::Min(80, $findText.Length)))"
        }
        foreach ($storyType in @($doc.StoryRanges | ForEach-Object { $_.StoryType } | Select-Object -Unique)) {
            ...
            while ($range -ne $null) {
                ...
                [void]$find.Execute($findText, $false, $false, $false, $false, $false, $true, 0, $false, $replaceText, 2)
