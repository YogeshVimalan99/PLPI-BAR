...
try{
  ...
  foreach($section in @($doc.Sections)){
    foreach($header in @($section.Headers)){
      foreach($paragraph in @($header.Range.Paragraphs)){
        ...
        if($plain -like 'Design Specification:*'){$r=$paragraph.Range.Duplicate;$r.End=$r.End-1;$r.Text='Design Specification: PLPI Batch Record Automation Phase 2';$r.Font.Name='Verdana';$r.Font.Size=[single]10;$r.Font.Bold=1;$r.Font.Color=6291456}
        elseif($plain -match '^(VMP/|PLPI/BAR/)'){$r=$paragraph.Range.Duplicate;$r.End=$r.End-1;$r.Text='PLPI/BAR/DS/01/v1';$r.Font.Name='Verdana';$r.Font.Size=[single]10;$r.Font.Bold=1;$r.Font.Color=6291456}
      }
    }
  }
  ...
  for($row=1;$row -le 5;$row++){
    ...
    foreach($cell in @($approval.Cell($row,2),$approval.Cell($row,4))){$b=$cell.Range.Paragraphs.Item(1).Borders.Item(-3);$b.LineStyle=1;$b.LineWidth=4;$b.Color=6291456}
  }
...
}finally{if($doc){$doc.Close($false)};$word.Quit();if($doc){[void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($doc)};[void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($word);[GC]::Collect();[GC]::WaitForPendingFinalizers()}
Write-Output 'Applied the FS/URS-style approval and revision front matter.'
