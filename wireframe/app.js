...
function renderLabelPrintingDocumentList(product) {
  ...
  const renderDocuments = (documents) => documents.map((item) => {
    ...
    return `<button class="label-document-link" type="button" ${actionAttributes}><span aria-hidden="true">▣</span>${document.buttonLabel}</button>`;
  }).join("");
  return `
...
function confirmQuantityPrint() {
  ...
  const lineCompletedNow = type === "label" && remainingAfter === 0;
  const itemsPrinted = lineCompletedNow
    ? Array.from(new Set([...(existing.itemsPrinted || []), itemLabel]))
    : [...(existing.itemsPrinted || [])];
  const itemCompletions = {
    ...(existing.itemCompletions || {}),
    ...(lineCompletedNow
      ? { [itemLabel]: { user: printedBy, dateTime: printedAt, status: "Completed" } }
      : {})
  };
  const nextRecord = {
    ...existing,
    ...
    itemLastPrints: {
      ...
      [itemLabel]: { user: printedBy, dateTime: printedAt, status: remainingAfter === 0 ? "Completed" : "Active" }
    },
    itemsPrinted,
    itemCompletions,
    printedAt
  ...
  savePrintRecord(type, batchNumber, nextRecord);
  auditPrinterAction(`${itemLabel} Printed`, product, `Print run quantity: ${quantity}; remaining quantity: ${remainingAfter}; printer: ${printer}; printed by ${printedBy} at ${printedAt}; line status: ${lineCompletedNow ? "Done" : "Active"}.`);
  closePrintPreview();
  renderStage("label-printing");
  statusMessage.textContent = lineCompletedNow
    ? `${itemLabel} required quantity printed and automatically marked Done by ${printedBy}. Progress was saved for the next printer.`
    : remainingAfter === 0
...
function markPrintLineDone(type, batchNumber, itemLabel) {
  ...
  const nextRecord = {
    ...
    itemCompletions: {
      ...(existing.itemCompletions || {}),
      [itemLabel]: { user: completedBy, dateTime: completedAt, status: "Completed" }
    }
  };
  nextRecord.printed = getPrintRequirements(type, product).every((item) => isPrintItemDone(nextRecord, item.label));
  savePrintRecord(type, batchNumber, nextRecord);
...
function getPackingRequiredDocuments() {
  return [
    ...
    { id: "additional-files", name: "Additional Files", required: false }
  ];
}
function getRpRequiredDocuments() {
  return getPackingRequiredDocuments().filter((doc) => doc.id !== "po" && doc.required !== false);
}
...
function renderPackingRpDocuments(stage) {
  ...
  const optionalDocs = getPackingRequiredDocuments().filter((doc) => doc.id !== "po" && doc.required === false);
  ...
  if (Object.keys(rpPackWorkflows).length === 0) {
    rpPackWorkflows["C13719"] = {
      ...
      documents: [
        ...
        { id: "additional-files", name: "Additional Files", required: false, uploaded: false, fileName: "", verified: false }
      ]
    };
  }
...
${product.routeType === "Reboxing" ? renderChangeOfPackSizeForm(product.batch, false, stage, true) : ""}
...
function openGeneratedBarPreview(batchNumber, allowPrint = false) {
  ...
  if (product && !generatedBarRecords[batchNumber] && generatedBatchNumbers.includes(batchNumber)) {
                    ...
                    ${labelRequirements.map((item) => {
                      ...
                      return `
                        <tr class="label-print-row ${done ? "label-print-row-complete" : "label-print-row-pending"}">
                          ...
                          <td><button class="classic-button row-print-button ${done ? "line-done-button" : ""}" type="button" disabled>${done ? "Done" : "Pending"}</button></td>
                          <td>${completion.user
...
changeOfPackSizeData[batch][section + "Initials"] = "";
  ...
  if (generatedBarPrintTrigger) {
    openGeneratedBarPreview(generatedBarPrintTrigger.dataset.printGeneratedBar, true);
    return;
  }
