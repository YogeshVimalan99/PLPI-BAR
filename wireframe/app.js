...
function areAllRpDocumentsVerified(poNo) {
  ...
  const sharedViewed = rpSharedDocumentIds.every((docId) => getRpTaskViewedDocs(primaryWorkflow)[docId] === true);
  const individualDocIds = getRpRequiredDocuments().map((doc) => doc.id).filter((docId) => !rpSharedDocumentIds.includes(docId));
...
function getRpTaskFilteredPoKeys() {
  ...
  return Object.keys(rpPackWorkflows).filter((poKey) => {
    ...
    const readyForRp = po.rpDocumentsQueueCreated === true && (po.status === "Signed Off" || po.signoff) && po.status !== "Rejected by RPi" && po.status !== "RPi Approved" && po.status !== "Waiting for QA decision" && !po.checklistExceptionStatus && !rpApprovalSignoffs[poKey] && isMergePrimary;
...
function getPackingDocCounts(po) {
  const docs = po && po.documents ? po.documents.filter((doc) => getRpRequiredDocuments().some((required) => required.id === doc.id)) : [];
  return { uploaded: docs.filter((doc) => doc.uploaded).length, verified: docs.filter((doc) => doc.verified).length, total: getRpRequiredDocuments().length };
}
...
function renderPackingRpDocuments(stage) {
  ...
  const poKeys = Object.keys(rpPackWorkflows).filter((poKey) => {
    ...
    if (!workflow || workflow.rpDocumentsQueueCreated !== true || workflow.status === "RPi Approved" || rpApprovalSignoffs[poKey] || workflow.checklistExceptionStatus) return false;
  ...
  if (!selectedRpPackPo) {
    const poListHtml = poKeys.map((poKey) => {
      ...
      const totalCount = poNos.length * getRpRequiredDocuments().length;
      return `
  ...
  const primaryWorkflow = rpPackWorkflows[primaryPoNo];
  const requiredDocs = getRpRequiredDocuments();
  ...
  const renderDocumentStatusRows = (entries) => {
    ...
    `;
    return `${renderRow("uploaded-files-row", "Uploaded files", `${uploadedEntries.length} available`, uploadedEntries, "No files uploaded yet")}${renderRow("remaining-files-row", "Remaining files", `${remainingEntries.length} outstanding`, remainingEntries, "All required files uploaded", true)}`;
  };
  ...
  const readyToSend = sharedUploaded && individualUploaded && !allSignedOff;
  const signoff = primaryWorkflow.signoff;
  return `
    <div class="packing-list-panel rp-documents-panel packing-rp-documents-panel">
      <div class="rp-documents-header">
        <button class="classic-button" type="button" data-rp-docs-back>Back to PO List</button>
        <div><h3>RPi Documents Dashboard - ${mergeGroup ? `Merged POs ${poNos.join(", ")}` : `PO ${activePo.poNo}`}</h3><p>${mergeGroup ? "Upload the shared and PO-specific files below." : "Upload the required document set before sign-off."}</p></div>
        <span class="viewed-badge ${allSignedOff ? "viewed" : "pending"}">${allSignedOff ? "Sent to RPi" : "Document set in progress"}</span>
      </div>
      ...
      <div class="signature-grid rp-doc-signoff-row">
        ...
        <div class="signoff-action process-signoff-action"><button class="classic-button primary" type="button" id="rp-documents-signoff-button" ${readyToSend ? "" : "disabled"}>Send PO set to RPi</button></div>
      </div>
    </div>
  `;
}
function updateRpDocumentsAvailability() {
  ...
  const sharedIds = getRpMergeGroup(primaryPoNo) ? rpSharedDocumentIds : [];
  const sharedUploaded = sharedIds.every((docId) => primaryWorkflow.documents.find((doc) => doc.id === docId)?.uploaded);
  const individualIds = getRpRequiredDocuments().map((doc) => doc.id).filter((docId) => !sharedIds.includes(docId));
  ...
  button.disabled = !(sharedUploaded && individualUploaded) || poNos.every((poNo) => rpPackWorkflows[poNo].status === "Signed Off");
  button.title = button.disabled ? "Upload all required documents before sign-off." : "Ready to send the document set to RPi";
}
...
function completeRpDocumentsSignoff() {
  ...
  poNos.forEach((poNo) => {
    const workflow = rpPackWorkflows[poNo];
    if (workflow) { workflow.status = "Signed Off"; workflow.signoff = signoff; }
  });
...
function renderRpPackWork(stage) {
...

  {
    ...
    const visibleActiveDocuments = activePo.documents.filter((doc) => doc.id !== "po");
    ...
    const mergedIndividualDocTilesByPo = Object.fromEntries(activePoNos.slice(1).map((memberPoNo) => {
      ...
      const tilesHtml = memberWorkflow.documents.filter((doc) => doc.id !== "po" && !rpSharedDocumentIds.includes(doc.id)).map((doc) => {
        const isUploaded = doc.uploaded || operationalDocIds.includes(doc.id);
...
function getLabelPrintingProducts() {
  return bnsProducts.filter(
    (product) => generatedBatchNumbers.includes(product.batch)
  );
}

function getLeafletPrintingProducts() {
  return bnsProducts.filter(
    (product) => labelPrintedBatchNumbers.includes(product.batch) && product.leafletRequired
  );
}
