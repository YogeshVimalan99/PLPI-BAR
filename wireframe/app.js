...
function renderPackingRpDocuments(stage) {
  ...
  const sharedTilesHtml = sharedRequiredDocs.map((required) => renderUploadTile(primaryWorkflow, required, `${poNos.length} POs Â· shared file`, true)).join("");
  const individualTilesByPo = Object.fromEntries(poNos.map((poNo) => {
    const workflow = rpPackWorkflows[poNo];
    return [poNo, individualRequiredDocs.map((required) => renderUploadTile(workflow, required, poNo)).join("")];
  }));
  const individualTilesHtml = poNos.map((poNo) => individualTilesByPo[poNo]).join("");
  ...
  const mergedPoSectionsHtml = mergeGroup ? `
    <div class="rp-document-section rp-common-files-section"><h4>Common files</h4><div class="rp-doc-tiles-grid">${sharedTilesHtml}</div></div>
    ${poNos.map((poNo) => `<div class="rp-document-section"><h4>PO ${poNo}</h4><div class="rp-doc-tiles-grid">${individualTilesByPo[poNo]}${packingListTilesByPo[poNo]}</div></div>`).join("")}
  ` : "";
  ...
  return `
    <div class="packing-list-panel rp-documents-panel packing-rp-documents-panel">
      ...
      ${mergeGroup ? mergedPoSectionsHtml : `<div class="rp-document-section"><h4>PO documents</h4><div class="rp-doc-tiles-grid">${individualTilesHtml}</div></div><div class="rp-document-section rp-packing-list-section"><h4>PLPI Packing List</h4><div class="rp-doc-tiles-grid">${packingListTilesHtml}</div></div>`}
      ...
      <div class="batchchecker-live-layout">
        ...
        <div class="batchchecker-list-wrap">
          <table class="batchchecker-po-list">
            <thead><tr><th>PO</th><th>Supplier</th><th>Lines</th><th>MFG Lot No</th><th>Quantity</th><th>Boxes</th><th>Status</th><th>Action</th></tr></thead>
            <tbody>${poRows || `<tr><td colspan="8" class="batchchecker-empty-state">No RPi approved POs are available yet. Complete RPi Task verification and sign off first.</td></tr>`}</tbody>
          </table>
        </div>
      </div>
    `;
  ...
  const tableRowsHtml = rows.map((row, index) => {
    ...
    return `
      <tr class="${selected ? "selected-row" : ""} ${isPclGenerated ? "pcl-generated-readonly-row" : ""}" data-batchchecker-row-index="${index}">
        <td class="batchchecker-select-cell"><button class="batchchecker-row-arrow" type="button" data-batchchecker-split="${index}" title="Edit quantity, expiry, and Batch No" ${readonlyAttr}>&#9656;</button><input type="checkbox" data-batchchecker-row-select="${index}" ${selected ? "checked" : ""} ${readonlyAttr}></td>
...

  {
    ...
    const mergedIndividualDocTilesByPo = Object.fromEntries(activePoNos.slice(1).map((memberPoNo) => {
      ...
      const tilesHtml = memberWorkflow.documents.filter((doc) => doc.id !== "po" && !rpSharedDocumentIds.includes(doc.id)).map((doc) => {
        ...
        return `
          <div class="rp-doc-tile" data-rp-view-file="${doc.id}" data-rp-po="${memberPoNo}" style="cursor: pointer;">
            ...
            <div class="rp-tile-status"><span class="rp-tile-badge ${isUploaded ? (isVerified ? "verified" : "pending") : "missing"}">${isUploaded ? (isVerified ? "Viewed" : "Pending View") : "No File"}</span></div>
            <div class="rp-tile-action">${isUploaded ? `<button class="classic-button link-button" type="button" style="font-size: 11px; padding: 4px 8px; width: 100%; pointer-events: none;">View File</button>` : `<span class="disabled-text">N/A</span>`}</div>
          </div>
        `;
      }).join("");
    ...
    const mergedPackingListTilesByPo = Object.fromEntries(activePoNos.slice(1).map((memberPoNo) => {
      ...
      const tileHtml = `
        <div class="rp-doc-tile" data-pl-preview-generated-packing-list data-rp-view-generated-packing-list data-pl-preview-po="${memberPoNo}" style="cursor: pointer; background-color: #fffbeb; border-color: #fcd34d;">
          <span class="rp-tile-index">PL</span>
          ...
          <div class="rp-tile-status"><span class="rp-tile-badge ${isViewed ? "verified" : "pending"}">${isViewed ? "Viewed" : "Pending View"}</span></div>
          <div class="rp-tile-action"><button class="classic-button link-button" type="button" style="font-size:11px;padding:4px 8px;width:100%;background-color:#d97706;color:white;pointer-events:none;">View File</button></div>
        </div>
      `;
...
function openPackingListPreview(poNoOverride = null, rowsOverride = null) {
  ...
  if (currentStageId === "rp-pack" && (rpiModuleView === "tasks" || rpiModuleView === "pack-creation") && rpPackWorkflows[poNo]) {
    ...
    if (currentStageId === "rp-pack" && rpiModuleView === "tasks") getRpTaskViewedDocs(rpPackWorkflows[poNo])["generated-packing-list"] = true;
  }
